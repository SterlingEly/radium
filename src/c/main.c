...
// Weather icon types (mapped from WMO code in weather_icon_for_code)
...
typedef struct {
  ...
  // Colors — time display (center LECO digits)
  GColor TimeColor;
  ...
  // Colors — leading-tick highlights
  GColor HourTipColor;    // leading hour tick (slightly brighter than LitHourColor)
  GColor MinuteTipColor;  // leading minute tick (noticeably brighter/whiter)

  // Colors — info lines (Line1=top-outer, Line2=top-inner, Line3=bot-inner, Line4=bot-outer)
  GColor Line1Color;
  GColor Line2Color;
  GColor Line3Color;
  GColor Line4Color;
...
static void prv_default_settings(void) {
  ...
  //   ticks/ring = MintGreen, time = White, tips = White, dim = DarkGreen
...
// Weather state — INT_MIN means "not yet received from phone"
...
static char s_time_buffer[8];      // "HH:MM"
...
// Fill a triangle wedge from center outward (used for tick marks)
...
// ICON DRAWING
//
// Small (OVERLAY_SMALL): icons are 11x11px, paired with GOTHIC_18_BOLD
//   cap height = 11px, font pad = 8px
// Large (OVERLAY_LARGE): icons are 14x14px, paired with GOTHIC_24_BOLD
//   cap height = 14px, font pad = 10px
//
// Icons are drawn at iy = y + font_pad so the top of the icon aligns
// with the top of the text cap. Small weather/steps shift up 1px.
// ============================================================
...
// Two overlapping shoe-prints for the steps icon
...
static void draw_steps_icon(GContext *ctx, int ox, int oy, GColor col, bool large) {
  if (!large) {
    draw_footprint(ctx, ox+5, oy-1, col, false); // right foot (shift up 1px)
...
static void draw_battery_icon(GContext *ctx, int ox, int oy, GColor col, int pct, bool large) {
  ...
  if (!large) {
    // 11x11: body 9x7 at oy+2, nub 2x3 at right
    ...
    // 14x14: body 12x10 at oy+1, nub 2x4
...
static void draw_cloud_icon(GContext *ctx, int ox, int oy, GColor col, bool large) {
  ...
  if (!large) {
    ...
    // 14px: bump column top at row 0, body rows 3-8
...
// font, icon size, and padding scale with the large flag.
// ============================================================
